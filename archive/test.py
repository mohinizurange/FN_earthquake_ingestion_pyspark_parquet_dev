...
def writeParquetDataintoGCS(spark,source_data_df, gcs_location,schema=None):
    if schema is None:
        source_data_df.coalesce(2).write.mode("overwrite").parquet(gcs_location)
        logging.info(f"Data written to {gcs_location}")
    else:
        source_data_df.coalesce(2).write.mode("overwrite").parquet(gcs_location,schema=schema)
        logging.info(f"Data written to {gcs_location}")
...
def dfSchema():
    # Define and return the schema
    silver_dataframe_schema = StructType([
        ...
        StructField("insert_date", TimestampType(), False)
    ])
    return silver_dataframe_schema
...
if __name__=='__main__':
    ...
    # extracted_data_df.show()
    extracted_data_df.printSchema()


    cur_timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    ## landing location
    eq_landing_gcs_loc = f"D:/Mohini Data Science/earthquake_ingestion_pyspark_parq_dev/landing/{cur_timestamp}/"
    ...
    eq_silver_gcs_loc =f'D:/Mohini Data Science/earthquake_ingestion_pyspark_parq_dev/silver/{cur_timestamp}/'
    silver_df_schema = dfSchema()
    new_clean_data_df = spark.createDataFrame(clean_data.rdd, schema=silver_df_schema)

    writeParquetDataintoGCS(spark, new_clean_data_df, eq_silver_gcs_loc)


    ###
    clean_raw_data = readParquetFilefromGCS(spark, eq_landing_gcs_loc)
    raw_data.show()
    raw_data.printSchema()
